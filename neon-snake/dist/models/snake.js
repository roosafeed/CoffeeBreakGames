...
export class Snake {
    constructor(length, startX = 0, startY = 0) {
        this.tiles = [];
        this.tiles = this.generateInitialTiles(length, startX, startY);
    }
    generateInitialTiles(length, startX, startY) {
        const initialTiles = [];
    ...
    getHeadingDirection() {
        ...
        if (this.tiles.length < 2)
            throw new Error("Not enough tiles to determine direction");
        const snake = this.getSnakeBody();
        const head = snake[0];
        const neck = snake[1];
        if (head.getX() === neck.getX()) {
            // either up or down
            if ((head.getY() + 1 === neck.getY())
                || (head.getY() > neck.getY() + 1 && neck.getY() === 0)) {
                return 'up';
            }
            else if ((head.getY() === neck.getY() + 1)
        ...
        if (head.getY() === neck.getY()) {
            ...
            if ((head.getX() + 1 === neck.getX())
                || (head.getX() > neck.getX() + 1 && neck.getX() === 0)) {
                return 'left';
            }
            else if ((head.getX() === neck.getX() + 1)
    ...
    moveForward(boardRows, boardCols) {
        const direction = this.getHeadingDirection();
    ...
    moveSnake(headNewX, headNewY, boardRows, boardCols) {
